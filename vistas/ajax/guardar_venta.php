...
// fin de numero de fatura
    ...
    while ($row = mysqli_fetch_array($sql)) {
        ...
        $insert_detail = mysqli_query($conexion, "INSERT INTO detalle_fact_ventas VALUES (NULL,'$id_factura','$numero_factura','$id_producto','$cantidad','$desc_tmp','$precio_venta_r','$precio_total','$id_stock')");
        ...
        $id_producto_kardex = $rww['producto_kardex'];
        ...
        guardar_salidas($date_added, $id_producto_kardex, $cantidad, $costo_producto, $saldo_total, $cant_saldo, $costo_saldo, $nuevo_saldo, $date_added, $users, $tipo);
// FIN
        // ACTUALIZA EN EL STOCK
        echo 'producto'.$id_producto;
        echo "select * from stock_lote where id_producto='" . $id_producto . "' and id_stock=".$id_stock;
        $sql2    = mysqli_query($conexion, "select * from stock_lote where id_producto='" . $id_producto . "' and id_stock=".$id_stock);
        $rw      = mysqli_fetch_array($sql2);
        $old_qty = $rw['stock']; //Cantidad encontrada en el inventario
        //echo $rw;
        $new_qty = $old_qty - $cantidad; //Nueva cantidad en el inventario
       // echo "UPDATE stock_lote SET stock='" . $new_qty . "' WHERE id_stock='" . $id_stock . "' and inv_producto=0";
        $update  = mysqli_query($conexion, "UPDATE stock_lote SET stock='" . $new_qty . "' WHERE id_stock=" . $id_stock . ""); //Actualizo la nueva cantidad en el inventario
